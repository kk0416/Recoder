...
int fdecodeplay::playflag=0;
int fdecodeplay::scrflag=0;
int fdecodeplay::posflag=0;
QString fdecodeplay::picname=nullptr;
...
void fdecodeplay::openViedoStream_play(QString filename)
{
    ...
    //2、打开：forcontent的地址 url的地址 预设一个格式 参数  返回0成功
    int res=avformat_open_input(&forcontent,filename.toStdString().c_str(),0,0);
    //判断是否打开
    if(res!=0)
    {
        qDebug()<<"打开视频失败"<<endl;
         ...
         if(pkt->stream_index==videoType)
         {
             ...
             if(got_picture_ptr!=0)
             {
                 ...
                 if(scrflag==1)
                 {
                     ...
                     QString str= QString("picturpath/Sc_Pic_%1.jpg").arg(dateTime);//图片名称
...
{
    this->registerFFmpeg();
//     QString filename="videopath/Data02-26-15-40-50.h264";
//    //实现边播放边编码
//    this->openViedoStream_jiema(filename);//录像
    qDebug()<<"播放标志位："<<playflag<<endl;
    if(playflag==1)
    {
        //    QString filename="videopath/Data02-26-15-40-50.h264";
        QString filename="videopath/"+picname;//从录像选择界面把图片名传回来
        qDebug()<<"run picname"<<picname<<endl;
        this->openViedoStream_play(filename);
        playflag=0;
    }
}
